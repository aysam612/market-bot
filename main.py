...
# الأرقام المخزنة مسبقاً (تم إضافة الرقم الخاص بك هنا مباشرة)
MEMORY_NUMBERS = [
    {
        "num_id": "zynx_num_1",
        "section": "شراء حساب جاهز",
        "country": "رقم أمريكي مميز +1 🇺🇸",
        "price": 2.50,
        "phone": "+13526419211",
...
# =====================================================================
# 👑 [لوحة التحكم المتقدمة المحدثة كلياً]
# =====================================================================
...
async def admin_panel_handler(event, state: FSMContext = None):
    ...
    builder = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ إضافة رقم جديد", callback_data="admin_auto_add_num"), InlineKeyboardButton(text="🗑 حذف كافة الأرقام", callback_data="admin_delete_all_nums")],
        [InlineKeyboardButton(text="⚙️ إدارة وتعديل الأرقام الحالية", callback_data="admin_manage_nums")],
        [InlineKeyboardButton(text="📅 تصفية السجلات والتواريخ", callback_data="admin_date_control_menu")],
        ...
        [InlineKeyboardButton(text="📊 إحصائيات البوت الشاملة", callback_data="admin_stats")],
        ...
        [InlineKeyboardButton(text="🎯 تعيين رصيد", callback_data="admin_set_balance"), InlineKeyboardButton(text="🔍 استعلام مستخدم", callback_data="admin_check_user")],
...
@dp.callback_query(F.data == "admin_delete_all_nums")
async def admin_delete_all_nums(callback: CallbackQuery):
    if callback.from_user.id != DEFAULT_ADMIN_USER_ID:
        return
    global MEMORY_NUMBERS
    MEMORY_NUMBERS = []
    await callback.answer("🗑 تم حذف جميع الأرقام من المتجر بنجاح!", show_alert=True)
...
async def admin_date_control_menu(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 عرض عمليات الشراء والسجلات", callback_data="admin_view_purchases_by_date")],
...
async def proc_country(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ التفاصيل: `{country_details}`\n\n🔢 الآن أرسل معرف الرقم الأساسي (ID) بالإنجليزية (مثال: `num2`):", parse_mode="Markdown")
